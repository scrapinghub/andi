...
def test_cannot_be_provided():
    ...
    with pytest.raises(andi.NonProvidableError) as ex_info:
        andi.plan(
            WithC, is_injectable={B}, externally_provided={A}, full_final_kwargs=True
        )
...
def test_plan_with_optionals_pipe():
    def fn(a: str | None):
        assert a is None
        return "invoked!"
...
def test_plan_with_optionals_and_union_pipe():
    def fn(str_or_b_or_None: str | B | None):
        return str_or_b_or_None
    ...
    plan = andi.plan(fn, is_injectable={type(None)}, externally_provided={B})
    assert isinstance(build(plan)[fn], B)
    ...
    assert error_causes(ex_info) == [
        (
            ...
            [
                NonInjectableOrExternalErrCase(
                    "str_or_b_or_None", fn, [str, B, type(None)]
                )
            ],
        )
    ]
...
def test_externally_provided():
    ...
    plan = andi.plan(E.__init__, is_injectable=[], externally_provided=ALL)
    assert dict(plan.dependencies) == {B: {}, C: {}, D: {}}
    assert _final_kwargs_spec(plan) == {"b": B, "c": C, "d": D}
    assert plan.full_final_kwargs
    ...
    plan = andi.plan(E, is_injectable=ALL, externally_provided={A, B, C, D})
    ...
    plan = andi.plan(E, is_injectable=ALL, externally_provided={A, B, D})
...
def test_plan_for_func():
    ...
    with pytest.raises(andi.NonProvidableError) as ex_info:
        andi.plan(
            fn, is_injectable=ALL, externally_provided=[A], full_final_kwargs=True
        )
    assert error_causes(ex_info) == [
...
def test_plan_non_annotated_args():
    ...
    plan = andi.plan(
        WithNonAnnArgs.__init__, is_injectable=ALL, externally_provided={A}
    )
    ...
    plan_class = andi.plan(WithNonAnnArgs, is_injectable=ALL, externally_provided=[A])
    ...
    o = WithNonAnnArgs(non_ann=None, non_ann_kw=None, **plan.final_kwargs(instances))
    ...
    assert error_causes(ex_info) == [
        ...
        ("non_ann_kw", [LackingAnnotationErrCase("non_ann_kw", WithNonAnnArgs)]),
    ]
...
def test_plan_non_injectable_args():
    ...
    plan = andi.plan(
        WithNonInjArgs.__init__, is_injectable=ALL, externally_provided={A}
    )
    ...
    plan_class = andi.plan(WithNonInjArgs, is_injectable=ALL, externally_provided=[A])
    assert plan_class.dependencies == plan.dependencies
    assert _final_kwargs_spec(plan_class) == _final_kwargs_spec(plan)
    assert not plan.full_final_kwargs
    ...
    instances = build(plan.dependencies, instances_stock={A: ""})
    o = WithNonInjArgs(non_inj=None, non_inj_kw=None, **plan.final_kwargs(instances))
    ...
    assert error_causes(ex_info) == [
        (
            ...
            [
                NonInjectableOrExternalErrCase(
                    "non_inj", WithNonInjArgs, types=[str, type(None)]
                )
            ],
        ),
        (
            ...
            [
                NonInjectableOrExternalErrCase(
                    "non_inj_kw", WithNonInjArgs, types=[str, type(None)]
                )
            ],
        ),
    ]
...
def test_plan_nested_non_injectable_args():
    ...
    assert error_causes(ex_info) == [
        (
            ...
            [
                NonInjectableOrExternalErrCase(
                    "non_inj_parent", WithNonInjArgs, types=[NonInjParent]
                )
            ],
        ),
    ]
...
def test_plan_overrides(recursive_overrides):
    ...
    if recursive_overrides:
        ...
        assert plan == [(B, {}), (C, {"a": B, "b": B}), (E, {"d": C, "c": C, "b": B})]
    else:
